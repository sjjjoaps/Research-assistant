...
本项目是一个面向学术文献场景的本地知识库助手，围绕“文献入库 -> 检索召回 -> 图谱增强 -> 问答/研究/Idea 生成”构建完整链路。当前项目已经融合了 `LightRAG` 的状态追踪、增量更新、双层检索等设计思路，也借鉴了 `RAG-Anything` 在多模态处理和工程组织上的部分机制。
...
### 12.1 开发约定
- 所有 Python 命令都使用 `llm_universe` 环境的完整解释器路径执行
- 新增 Prompt 统一放在 `prompt/` 目录
- 每完成一个阶段，同步更新：
  - `README.md`
  - `doc/开发步骤规划.md`
  - 必要代码注释
...
### 12.2 推荐开发流程
1. 阅读 [doc/开发步骤规划.md](./doc/开发步骤规划.md)
2. 选择当前阶段目标
3. 修改代码与测试
4. 运行相关测试
5. 更新 README 和阶段文档
...
当前项目以个人迭代开发为主，但建议按照以下方式提交改动：

1. 明确说明所属阶段和目标，例如 `Phase 4.3`、`Phase 5.2`
2. 优先提交小而完整的改动，不要混入大量无关重构
3. 新增功能时同时补测试和文档
4. 如修改 Prompt、检索策略、入库主流程，提交说明中应写明行为变化
...
### 14.5 某些测试在 Windows 下受临时目录权限影响
- 优先跑与本次改动直接相关的测试
- 避免把系统临时目录权限问题误判为业务逻辑回归

---

## 15. 后续规划

后续工作以 [doc/开发步骤规划.md](./doc/开发步骤规划.md) 为准，当前已完成到：
- Phase 5.3：状态展示完善
