...
### 文档状态展示（Phase 5.3）
- `/documents` 列表接口现在直接返回当前处理步骤、chunk/实体/关系数量、最近错误和更新时间
- Streamlit「文献管理」页面会直接展示这些状态字段，便于观察入库进度和失败位置
- 详细状态面板会对 `failed` / `delete_failed` 状态突出显示最近错误
- 单文件入库新增后台启动与实时追踪，页面会在入库期间持续显示“解析中 / 切块中 / 提取元数据 / 写入索引 / 写入图谱”等阶段提示
