"""
测试 SessionManager（P5-Step4 原生化版本）
...
测试内容：
  ...
  6. 消息类型完整性：user / assistant / tool / assistant+tool_calls
  7. 压缩游标（covers_turn_ids 集合判定）：load() 仅返回 system + 新轮消息
...
def make_turn_messages(text: str = "Hello") -> list[dict]:
    """构造一组典型的 turn 消息（user + assistant）"""
    return [
        {"role": "user",      "content": text},
        {"role": "assistant", "content": f"回答：{text}"},
    ]


def make_tool_turn_messages() -> list[dict]:
    """构造包含 tool_calls 和 tool 消息的 turn 消息"""
    return [
        {
            "role": "assistant",
            "content": "",
            "tool_calls": [
                {
                    "id": "tc_001",
                    "type": "function",
                    "function": {"name": "retrieve_knowledge", "arguments": '{"query": "test"}'},
                }
            ],
        },
        {"role": "tool", "content": "检索到 3 个结果", "tool_call_id": "tc_001"},
        {"role": "assistant", "content": "根据检索结果，答案是..."},
    ]
...
def test_tool_message_roundtrip():
    ...
    assert ai_with_tool["role"] == "assistant"
    assert len(ai_with_tool.get("tool_calls", [])) == 1
    assert ai_with_tool["tool_calls"][0]["function"]["name"] == "retrieve_knowledge"
...
def test_compact_cursor_set_based():
    """
    验证 load() 通过 covers_turn_ids 集合跳过旧轮：
    - 手动写入 summary 记录并指定 covers_turn_ids
    - load() 应跳过被覆盖的旧轮，返回 1 条 system + 新轮消息
    """
    sm = SessionManager()
    sid = fresh_session_id()
    ...
    loaded = sm.load(sid)
    # 期望：1 条 system（summary）+ 新轮 2 条 = 3 条；旧轮 2 条被跳过
    assert len(loaded) == 3, f"期望 3 条消息，实际 {len(loaded)}"

    assert loaded[0]["role"] == "system", f"期望 system，实际 {loaded[0]['role']}"
...
def test_serialize_deserialize():
    """P5-Step4：序列化/反序列化为 dict 直通，验证 role 规范化。"""
    msgs = [
        ...
        {"role": "tool", "content": "找到 5 篇文献", "tool_call_id": "t1"},
    ]
    for msg in msgs:
        data   = _serialize_message(msg)
        result = _deserialize_message(data)
        assert result["role"] == msg["role"], f"role 不一致: {result['role']} vs {msg['role']}"
        assert result["content"] == msg["content"], f"content 不一致"

    # 旧版 LangChain role 兼容性
    old_human = {"role": "human", "content": "旧格式"}
    assert _deserialize_message(old_human)["role"] == "user"

    old_ai = {"role": "ai", "content": "旧格式回答"}
    assert _deserialize_message(old_ai)["role"] == "assistant"
...
if __name__ == "__main__":
    print("=" * 60)
    print("P5-Step4  SessionManager 测试（原生化版本）")
    print("=" * 60)
