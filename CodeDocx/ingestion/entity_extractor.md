...
## 主体功能
从 chunk 文本中抽取学术实体与关系，写入 Neo4j 图数据库。
...
| `EntityExtractionStats` (dataclass) | 统计信息：处理 chunk 数 / 创建实体数 / 关系数 |
| `EntityExtractor.extract_for_document(chunks, file_path)` | 主入口：逐 chunk 调用 LLM 抽取，写入 Neo4j，返回统计 |
